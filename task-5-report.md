...
- Serialized anonymous Supabase fallback per auth client in `src/lib/auth.ts` with a shared in-flight promise, so concurrent callers without a session reuse one `signInAnonymously()` request and the lock clears after either resolve or reject.
...
- Added auth regressions in `src/lib/auth.test.ts` covering concurrent anonymous-session callers sharing a single sign-in and retry behavior after a rejected in-flight sign-in clears.
