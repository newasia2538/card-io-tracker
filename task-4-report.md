...
- Hardened USD rate error classification with a typed malformed-response sentinel in `api/internal/rates`, so provider availability failures still map to `503 rate_unavailable`, while malformed Frankfurter payloads now map to `502 upstream_unavailable`.
...
- Service tests cover canonical USD conversion, rate-unavailable behavior, malformed provider-rate `502` mapping, real Frankfurter malformed JSON/date/base/quote/rate regressions, not-found mapping, and exchange-rate stale/provider-date output.
- Handler tests cover bearer auth, list/create/update/delete flows, `503` for USD writes without a usable rate, `502` for malformed upstream rate payloads from the real Frankfurter provider path, trailing top-level JSON rejection, stable error codes, and exchange-rate responses.
